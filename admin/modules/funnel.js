...
import { $, escapeHtml as esc, formatDate, numeric } from '../ui.js?v=20260903-phase5';
...
export async function refreshAdminFunnel(){
  ...
  try{
    ...
    if(current===sequence){root.textContent='Funnel teď nelze načíst. ';const retry=document.createElement('button');retry.textContent='Zkusit znovu';retry.type='button';retry.onclick=()=>void refreshAdminFunnel();root.append(retry)}
