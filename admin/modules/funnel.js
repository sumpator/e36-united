...
import { $, escapeHtml as esc, formatDate, numeric } from '../ui.js?v=20260920-registration-ui-r1';
...
export function renderAdminFunnel(payload){
  const root=$('[data-admin-funnel]');if(!root||root.contains(document.activeElement))return false;
  const eventId=adminState.selectedEventId,opened=[...root.querySelectorAll('details')].map(d=>d.open);
    const counts=payload.counts||{},details=payload.details||{};
    const metrics=[['members','Členské účty','Všechny existující D1 profily, bez ohledu na event.'],['incomplete','Nedokončené registrace','Nově sledovaná Firebase identita bez úspěšného profilu. Globálně.'],['created','Vytvořené plány','Unikátní zaznamenané plány pro vybraný event.'],['claimed','Otevřené / přiřazené','Plán otevřený v Můj United a přiřazený k účtu.'],['unclaimed','Nepřiřazené plány','Zaznamenané plány bez otevření v Můj United.'],['converted','S registrací','Plány propojené s úspěšně uloženou registrací.'],['claimedWithoutReservation','Přiřazené bez registrace','Otevřené plány, které zatím nemají zaznamenanou konverzi.']];
