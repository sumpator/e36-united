import { canonicalMemberLink } from '../member-detail.js?v=20260920-registration-ui-r1';
import {showReservationMedia,clearReservationMedia} from '../reservation-media.js?v=20260920-registration-ui-r1';
import {listChanged,renderListPagination} from '../lists.js?v=20260920-registration-ui-r1';
import { adminCommand, editorProtected, allowAdminNavigation, forgetAdminEditor } from '../editors.js?v=20260920-registration-ui-r1';
import { accommodationVisualMarkup, bindAccommodationVisualFallbacks } from '../../accommodation-visual.js?v=20260920-registration-ui-r1';
import { RESERVATION_DETAIL_FILTERS, RESERVATION_PRIMARY_FILTERS, RESERVATION_VIEW_MODES, adminItemPayment, filterAdminPayments, filterAdminReservations, paymentMatchesFilter, reservationMatchesFilter } from '../../admin-view-model.js?v=20260920-registration-ui-r1';
import { apiBaseUrl, apiRequest } from '../api.js?v=20260920-registration-ui-r1';
import { renderAttentionCounts } from './dashboard-events.js?v=20260920-registration-ui-r1';
...
import { $, $$, attendanceLabel, attendanceShortLabel, escapeHtml, formatDate, formatMoney, numeric, paymentLabel, paymentQrSvg, recordsLabel, rememberSessionChoice, statusLabel, toast } from '../ui.js?v=20260920-registration-ui-r1';
...
import {mergeReservation} from '../confirmed-state.js?v=20260920-registration-ui-r1';
...
function reservationActions(item){
  const actions=[
    ...
    {status:'approved',label:'Schválit registraci',className:'approve'},
    {status:'rejected',label:'Zamítnout registraci',className:'reject'},
  ];
...
function reservationRequestCapacityMarkup(request){
  ...
  return `<div class="admin-request-capacity${capacity.available?' is-available':' is-conflict'}" data-request-capacity>
    ...
    <dl><div><dt>Celkem</dt><dd>${limited?numeric(capacity.unitsTotal):'Bez limitu'}</dd></div><div><dt>Aktuálně obsazeno</dt><dd>${numeric(capacity.occupiedUnits)}</dd></div><div><dt>Aktuálně volno</dt><dd>${limited?numeric(capacity.freeUnits):'Bez limitu'}</dd></div><div><dt>Tato registrace</dt><dd>${numeric(capacity.currentReservationUnits)}</dd></div><div><dt>Návrh</dt><dd>${numeric(capacity.proposedUnits)}</dd></div><div><dt>Čistý rozdíl</dt><dd>${net>0?'+':''}${numeric(net)}</dd></div><div><dt>Po schválení</dt><dd>${numeric(capacity.occupiedAfterApproval)} obsazeno${limited?` · ${numeric(capacity.freeAfterApproval)} volno`:''}</dd></div></dl>
...
function renderReservationList(){
  ...
  if(!reservations.length){list.innerHTML='<div class="admin-empty">Aktuálním filtrům a hledání neodpovídá žádná registrace.</div>';return}
  const quick=adminState.reservationViewMode==='quick';
  const headers=quick?'<th>Člen</th><th>Registrace</th><th>Celkem</th><th>Uhrazeno</th><th>Platba</th><th>Bilance</th><th>Aktualizace</th><th></th>':'<th>Člen</th><th>E-mail</th><th>Přezdívka</th><th>Posádka</th><th>Účast</th><th>Ubytování</th><th>Celkem</th><th>Uhrazeno</th><th>VS</th><th>Stavy</th><th>Aktualizace</th><th></th>';
...
<header><span class="admin-kicker">Detail registrace</span><div><small data-command-reservation-status class="admin-badge admin-badge--${escapeHtml(item.status)}">${escapeHtml(item.cancellationPending?'Zrušení čeká na schválení':item.changePending?'Změna čeká na schválení':statusLabel(item.status))}</small><small class="admin-badge admin-payment--${escapeHtml(payment.status)}">${escapeHtml(payment.overdue&&payment.remainingCzk>0?'Po splatnosti':paymentLabel(payment.status))}</small></div><h2 id="admin-reservation-drawer-title">${escapeHtml(member.nickname||member.name||'Člen United')}</h2><p>${escapeHtml(member.name||'Jméno neuvedeno')} · ${escapeHtml(member.email||'E-mail neuveden')}</p><p class="command-reservation-reference">Registrace ${escapeHtml(item.id)}</p></header>
    ...
    <section class="admin-payment-editor">${payment.testMode?'<div class="payment-test-warning">TESTOVACÍ PLATBA – NEPLAŤTE</div>':''}<div class="admin-payment-editor-grid"><div><span class="admin-kicker">Finance</span><h3>${escapeHtml(payment.overdue&&payment.remainingCzk>0?'Platba po splatnosti':reservationDifference(item))}</h3><dl><div><dt>Cena registrace</dt><dd>${escapeHtml(formatMoney(payment.amountDueCzk))}</dd></div><div><dt>Evidovaně uhrazeno</dt><dd>${escapeHtml(formatMoney(payment.amountPaidCzk))}</dd></div><div><dt>${payment.status==='overpaid'?'Přeplatek':payment.status==='underpaid'?'Doplatek':'Bilance'}</dt><dd>${escapeHtml(payment.status==='overpaid'?formatMoney(payment.overpaymentCzk):formatMoney(payment.remainingCzk))}</dd></div><div><dt>VS</dt><dd>${escapeHtml(payment.variableSymbol||'—')}</dd></div><div><dt>Účet</dt><dd>${escapeHtml(payment.accountDisplay||'—')}</dd></div><div><dt>Splatnost</dt><dd>${escapeHtml(formatDate(payment.deadline,false))}</dd></div></dl><label><span>SKUTEČNĚ UHRAZENO (KČ)</span><input data-payment-amount max="10000000" min="0" step="1" type="number" value="${numeric(payment.amountPaidCzk)}"/></label><div class="admin-payment-actions"><button class="admin-button admin-button--primary" data-payment-save type="button">Uložit platbu <span>→</span></button><button class="admin-button" data-payment-full type="button">Označit plně uhrazeno</button></div></div>${qr?`<div class="admin-payment-qr"><div>${qr}</div><small>${escapeHtml(payment.message||'')}</small></div>`:''}</div></section>
    ...
    <div class="admin-review admin-reservation-drawer-review"><label><span>Interní poznámka</span><input maxlength="1000" data-review-note placeholder="Vidí pouze Admin" value="${escapeHtml(item.reviewNote||'')}"/></label><label><span>Zpráva pro člena</span><textarea maxlength="1000" rows="3" data-reservation-member-comment placeholder="Zobrazí se členovi u registrace">${escapeHtml(item.memberComment||'')}</textarea></label><div class="admin-notes-actions"><button class="admin-button admin-button--primary" data-reservation-notes-save type="button">Uložit poznámky</button></div></div>
...
export async function updateReservation(card,status,reloadEventData){
  ...
  try{
    ...
    const messages={pending:'Registrace byla vrácena k posouzení.',approved:'Registrace byla schválena.',rejected:'Registrace byla zamítnuta.'};toast(messages[status]||'Stav registrace byl změněn.');
  ...
  catch(error){if(error.status===403){setDenied();return}toast(error.message||'Registraci se nepodařilo změnit.')}finally{if(button)button.disabled=false}
}
...
export async function reviewReservationRequest(card,decision){
  ...
  try{
    ...
    toast(decision==='approved'?'Žádost byla schválena.':'Žádost byla zamítnuta; původní registrace zůstala beze změny.');
