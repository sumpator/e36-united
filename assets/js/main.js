(() => {
  ...
  if (communityUpload) {
    ...
    form?.addEventListener('submit', async e => {
      ...
      setStatus('Nahrávám fotografie…');
      try {
        const response = await fetch(form.action,{method:'POST',body:new FormData(form),headers:{'Accept':'application/json'}});
        const data = await response.json().catch(() => ({}));
        if (!response.ok || !data.ok) throw new Error(data.message || 'Upload se nepodařilo dokončit.');
        setStatus(data.message || 'Hotovo. Fotky jsou nahrané a čekají na schválení.','success');
        form.reset();
        preview.innerHTML = '';
        if (count) count.textContent = `0 / ${maxFiles} vybráno`;
      } catch (error) {
        setStatus(error.message || 'Upload se nepodařil. Zkus to prosím znovu.','error');
      } finally {
  ...
  const fallbackFocus = {
    exterior:{scale:1.48,x:1,y:0,focusX:52,focusY:49},
    interior:{scale:2.12,x:-7,y:5,focusX:43,focusY:47},
    paint:{scale:2.32,x:10,y:2,focusX:59,focusY:45},
    wheels:{scale:2.72,x:-22,y:17,focusX:31,focusY:68},
    engine:{scale:2.28,x:24,y:-14,focusX:70,focusY:33}
  };
  // Fail-safe: the inspection camera must still work even if the optional
  // category data file is missing or cached incorrectly.
  ...
  let activeInspectionMode = 'overview';
  ...
  if (lab) {
    ...
    const inspectionData = {
      overview:{ label:'CELÉ AUTO', title:'Celek rozhoduje stejně jako detail.', copy:'Porota posuzuje vůz v pěti oblastech. Každá má vlastní body a společně tvoří výsledný dojem auta.', kicker:'SHOW & SHINE VIEW' },
      exterior:{ label:'EXTERIÉR', title:'Karoserie musí fungovat jako celek.', copy:'Zpracování / spasování, koroze, originalita dílů a celkový dojem.', kicker:'01 / EXTERIÉR' },
      interior:{ label:'INTERIÉR', title:'Interiér ukáže péči i originalitu.', copy:'Čistota, originalita, použité materiály, čalounění, prvky výbavy, stav a celkový dojem.', kicker:'02 / INTERIÉR' },
      paint:{ label:'LAK', title:'Lak neokecáš.', copy:'Provedení lakování, originalita barvy, původní lak a jeho stav.', kicker:'03 / LAKOVÁNÍ' },
      wheels:{ label:'KOLA', title:'Kola mění charakter celého auta.', copy:'Volba kol, vzhled, stav, lak, leštění, čistota a případné poškození.', kicker:'04 / KOLA' },
      engine:{ label:'MOTOR', title:'Kapota nahoru.', copy:'Vzhled motorového prostoru, čistota, provedení, originalita a úpravy.', kicker:'05 / MOTOR A JEHO PROSTOR' }
    };
    ...
    const cameraFor = (category, mode) => {
      if (mode === 'overview') return {scale:1,x:0,y:0,focusX:50,focusY:50};
      return category.focus?.[mode] || {scale:1.7,x:0,y:0,focusX:50,focusY:50};
    };
    ...
    const setInspection = (mode, {instant=false} = {}) => {
      ...
      stage.classList.toggle('is-tracking', mode !== 'overview');
      ...
      stage.style.setProperty('--cam-rot', `${mode === 'overview' ? 0 : (camera.x || 0) * .008}deg`);
      ...
      if (scanStatus) scanStatus.textContent = mode === 'overview' ? 'STANDBY' : 'TRACKING';
      ...
      if (detailSrc && detailImg) {
        ...
        }, reduceMotion || instant ? 0 : 700);
      }

      settleTimer = setTimeout(() => {
        if (activeInspectionMode !== mode) return;
        stage.classList.add('is-settled');
        if (scanStatus) scanStatus.textContent = mode === 'overview' ? 'STANDBY' : 'LOCKED';
    ...
    const setCategory = (key, {instant=false} = {}) => {
      ...
      const applyImage = () => {
        ...
        setInspection('overview',{instant:true});
      };
  ...
  if (unitedMap) {
    ...
    const flowAssets = {
      ...
      sleep: {
        'Chatka': {
          image:'assets/images/cabin-zbraslavice-user.png',
          fallback:'assets/images/map-cabin.svg',
