...
-- Include the already-deployed Production Feedback migration in the canonical snapshot.
-- This is schema documentation, not a new migration or historical data backfill.
...
-- Mailing C additive schema follows. Not yet deployed.
PRAGMA foreign_keys = ON;
...
ALTER TABLE mailing_campaigns ADD COLUMN provider_request_id TEXT;
...
ALTER TABLE mailing_campaign_recipients ADD COLUMN provider_email_id TEXT;
...
CREATE TABLE mailing_delivery_events (
  ...
  normalized_email TEXT NOT NULL,
  provider TEXT NOT NULL CHECK(provider='smtp2go'),
  provider_event_key TEXT NOT NULL,
  event_type TEXT NOT NULL CHECK(event_type IN ('sent','delivered','opened','click','soft_bounce','hard_bounce','blocked','unsubscribed','rejected','resubscribed')),
...
CREATE UNIQUE INDEX idx_mailing_recipient_provider_email ON mailing_campaign_recipients(provider_email_id) WHERE provider_email_id IS NOT NULL;
...
INSERT INTO schema_migrations(id,description) VALUES('2026-09-07-mailing-delivery','Mailing C preparation, SMTP2GO delivery and tracking');
...
-- UNITED LIVE v1. See forward migration 2026-09-22-united-live.sql.
ALTER TABLE events ADD COLUMN live_enabled INTEGER NOT NULL DEFAULT 0 CHECK(live_enabled IN (0,1));
ALTER TABLE gallery_submissions ADD COLUMN event_id TEXT REFERENCES events(id) ON DELETE SET NULL;
CREATE UNIQUE INDEX events_single_live ON events(live_enabled) WHERE live_enabled=1;
CREATE TABLE event_program_items (id TEXT PRIMARY KEY,event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE,day TEXT NOT NULL,starts_at TEXT NOT NULL,ends_at TEXT,title TEXT NOT NULL,venue TEXT,description TEXT,status TEXT NOT NULL DEFAULT 'scheduled' CHECK(status IN ('scheduled','changed','cancelled')),visible INTEGER NOT NULL DEFAULT 1 CHECK(visible IN (0,1)),sort_order INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE INDEX event_program_schedule ON event_program_items(event_id,visible,day,starts_at,sort_order,id);
CREATE TABLE event_live_judges (event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE,member_id TEXT NOT NULL REFERENCES members(id) ON DELETE CASCADE,created_by TEXT REFERENCES members(id) ON DELETE SET NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(event_id,member_id));
CREATE TABLE event_member_presence (event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE,member_id TEXT NOT NULL REFERENCES members(id) ON DELETE CASCADE,present INTEGER NOT NULL DEFAULT 1 CHECK(present IN (0,1)),confirmed_by TEXT REFERENCES members(id) ON DELETE SET NULL,confirmed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(event_id,member_id));
CREATE TABLE live_entries (id TEXT PRIMARY KEY,event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE,discipline TEXT NOT NULL CHECK(discipline IN ('show_shine','best_exhaust')),member_id TEXT NOT NULL REFERENCES members(id) ON DELETE CASCADE,car_id TEXT NOT NULL REFERENCES cars(id) ON DELETE RESTRICT,category TEXT,presented_at TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(event_id,discipline,car_id));
...
UPDATE events SET live_enabled=1 WHERE is_current=1 AND NOT EXISTS(SELECT 1 FROM events WHERE live_enabled=1);
INSERT INTO schema_migrations(id,description) VALUES('2026-09-22-united-live','UNITED LIVE event, program, presence, voting, jury and internal media foundation');
...
CREATE INDEX live_category_state_overview ON live_category_state(event_id,discipline,status,category);
INSERT INTO schema_migrations(id,description) VALUES('2026-09-22-united-live-categories','Server-owned Show & Shine category lifecycle without guessing legacy entries');
...
INSERT INTO schema_migrations(id,description) VALUES('2026-09-23-live-writes-enabled','LIVE schema verified and write fence released');
