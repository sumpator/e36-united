...
## 2026-09-12 — reservation-request visibility and capacity amendment

The accepted Stage 2 and Stage 3 figures above remain unchanged; this narrow amendment records only the incremental cost of completing the reservation change/cancellation workflow. The existing summary HTTP request now executes a two-statement D1 batch. Its additional statement is one event-scoped aggregate over reservations with a single join to the unique pending-request index. It returns only counts for new reservations, change requests and cancellation requests, and a de-duplicated overall attention count; it does not load queue rows and is not executed per reservation. Local `EXPLAIN QUERY PLAN` evidence is asserted in `tests/admin-budget-closure.test.mjs`: event lookup uses `idx_reservations_payment`, pending requests use `reservation_requests_one_pending`, and no pending-request table scan or N+1 query is permitted.

Capacity is queried only when an Admin opens a reservation detail containing a pending change request and again immediately before approval. The query is scoped to the reservation, event and proposed accommodation option, excludes the current reservation's existing allocation, and does not reserve capacity for a pending request. A capacity conflict returns before the existing atomic approval batch, so it produces no partial reservation or allocation write. Increasing capacity remains a separate, explicit Admin action; the request detail links to the exact accommodation option and requires a fresh recheck before approval.

Normal Member loading adds no HTTP request: the existing reservation-request projection includes the nullable acknowledgement timestamp. A successful, request-specific “Beru na vědomí” action adds one explicit POST, one conditional update and one authoritative readback. Request submission remains one explicit POST and does not allocate capacity. There is no new polling timer, background task, binary-media load, provider call or storage operation.

These statements describe local query shape and conservative structural impact, not measured Cloudflare `meta.rows_read`. No deployment, remote profiling, production migration or production write was performed for this amendment. The new nullable column requires the separately authorized migration `2026-09-12-reservation-request-acknowledgement.sql`; existing requests remain unacknowledged (`NULL`) until their member explicitly confirms an approved change.

## 2026-09-13 — Member Club profile boundary

This Member-facing addition does **not** change the accepted Admin Stage 2/3 envelopes, the 60/120/300-second coordinator policy or any Admin dashboard query. It introduces no polling. The existing `GET /api/united-club` HTTP request gains exactly one event-independent, `LIMIT 25` club-member list statement (24 visible rows plus a `hasMore` probe), with aggregate attendance and primary-photo selection inside that single statement; there is no per-member/N+1 request.

An explicit profile open is one new HTTP request and six D1 statements including the shared active-member guard: guard, target visibility lookup, then a bounded four-statement batch for confirmed history (24), Points, garage/photo rows (36) and approved gallery rows (24). A displayed car image is a separate lazy interaction: one HTTP request, guard plus one joined D1 ownership/visibility statement, then one private R2 GET. Authenticated gallery author linking is one request per gallery load, not per photo: guard plus one statement bounded to 72 supplied approved-photo IDs. Anonymous gallery loading remains the unchanged public request and receives no profile references.

Changing “Skrýt na klubu” reuses the existing explicit bootstrap/profile-save request and statement; it adds no background write. The one forward-only migration adds a default-false integer column and an active/visibility ordering index. It performs schema/index writes and consumes index storage; future visibility/status/created-at/member insert/delete changes maintain that index. No production row count or Cloudflare `meta.rows_read`/`rows_written` claim is made from these structural bounds, and no remote migration or profiling was performed.
