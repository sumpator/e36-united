...
const lightboxCounter = qs('[data-lightbox-counter]', lightbox);
const lightboxTitle = qs('[data-lightbox-title]', lightbox);
const lightboxThumbs = qs('[data-lightbox-thumbs]', lightbox);
let lightboxIndex = 0;
...
qs('.lightbox-prev', lightbox)?.addEventListener('click',()=>paintLightbox(lightboxIndex-1));
qs('.lightbox-next', lightbox)?.addEventListener('click',()=>paintLightbox(lightboxIndex+1));
...
control.addEventListener('click', () => { setInspection(mode); if (window.matchMedia('(max-width:680px)').matches) setTimeout(() => stage.scrollIntoView({behavior:'smooth',block:'nearest'}), 90); });
});
categoryCards.forEach(card => {
