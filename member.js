...
async function initFirebase(){
  if(!firebaseReady){setMode('LOCAL PREVIEW'); if(localStorage.getItem(sessionKey)==='1'){showApp();renderAll();applyPlannerDraft()}else showAuth(); return;}
  try{
    ...
    authMod.onAuthStateChanged(auth,async user=>{currentUser=user;if(user&&!user.isAnonymous){localStorage.setItem(sessionKey,'1');await loadRemote();showApp();await consumeConfirmationToken();applyPlannerDraft()}else{localStorage.removeItem(sessionKey);showAuth()}});
  }catch(e){console.warn('Firebase unavailable, using preview mode',e);setMode('LOCAL PREVIEW');}
}
...
function showAuth(){ $('[data-auth-view]').hidden=false; $('[data-app-view]').hidden=true; }
function showApp(){ $('[data-auth-view]').hidden=true; $('[data-app-view]').hidden=false; renderAll(); }
...
function renderReservation(){
  ...
  miniStatus.textContent=confirmed?'Potvrzeno':'Čeká na e-mail';year.textContent=r.year||'NEXT';title.textContent=r.title||'United rezervace';car.textContent=r.carSnapshot?`${r.carSnapshot.nickname||r.carSnapshot.model} · ${r.carSnapshot.body}`:'Auto není vybrané';
  $('[data-reservation-state-label]').textContent=confirmed?'Rezervace potvrzena':'Čeká na potvrzení e-mailem';$('[data-reservation-year]').textContent=r.year||'NEXT';$('[data-reservation-title]').textContent=r.title||'E36 United';
  $('[data-reservation-description]').textContent=confirmed?'Rezervace je potvrzená a napárovaná na konkrétní snapshot auta z tvojí garáže.':'Rezervace je uložená, ale aktivuje se až po kliknutí na potvrzovací odkaz v e-mailu.';
  const sleep = r.arrival==='Jen na otočku' ? 'Bez ubytování' : r.sleep;
  $('[data-reservation-summary]').innerHTML=`<div><small>AUTO</small><b>${esc(r.carSnapshot?.nickname||r.carSnapshot?.model||'—')}</b></div><div><small>PŘÍJEZD</small><b>${esc(r.arrival||'—')}</b></div><div><small>POSÁDKA</small><b>${esc(r.crew)} osoby</b></div><div><small>UBYTOVÁNÍ</small><b>${esc(sleep||'—')}</b></div><div><small>SHOW & SHINE</small><b>${esc(r.showshine)}</b></div><div><small>STATUS</small><b>${confirmed?'POTVRZENO':'ČEKÁ NA E-MAIL'}</b></div>`;
  if(mailState){mailState.classList.toggle('is-confirmed',confirmed);mailState.querySelector('span').textContent=confirmed?'E-mail potvrzen · rezervace je aktivní.':'Otevři e-mail od E36 United a potvrď rezervaci jedním kliknutím.'}
}
...
reservationForm?.addEventListener('submit',async e=>{
  ...
  if(firebase&&currentUser&&!currentUser.isAnonymous){
    ...
    data.reservation.previewConfirmation=true;await commit('Preview: rezervace čeká na potvrzení e-mailem');
  }
});
...
function applyPlannerDraft(){
  const raw=localStorage.getItem(plannerDraftKey);if(!raw||!reservationForm)return;
  try{
    ...
    if(reservationForm.elements.showshine){
      const map={'Chci soutěžit':'Ano','Jedu se podívat':'Ne','Možná':'Možná'};
      reservationForm.elements.showshine.value=map[draft.showshine]||draft.showshine||'Ne';
    }
    syncMemberSleep();openSection('reservation');localStorage.removeItem(plannerDraftKey);toast('Weekend Planner byl přenesen do profilu. Vyber auto a rezervaci potvrď.');
  }catch(err){console.warn(err);localStorage.removeItem(plannerDraftKey)}
}
