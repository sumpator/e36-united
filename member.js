...
const memberHelpContent={
  ...
  rating:{kicker:'MEMBER RATING',title:'Tvoje členská úroveň',copy:'Rating roste podle všech bodů, které jsi kdy získal: od 316i až po M POWER.'},
  ...
  'earn-attendance':{kicker:'ÚČAST NA SRAZU',title:'Ověřené United',copy:'Každý ověřený sraz = +1 bod. Celkem 3 ověřené srazy = +3 body navíc. Celkem 5 ověřených srazů = +3 body navíc.'},
  'earn-showshine':{kicker:'SHOW & SHINE',title:'Ověřené umístění',copy:'3. místo = +1. 2. místo = +2. 1. místo = +3. Best of the Best = +1 navíc. Nej zvuk výfuku = +1 navíc.'},
  'earn-photos':{kicker:'NAHRÁVÁNÍ FOTEK',title:'Schválené komunitní fotky',copy:'5 schválených = +1. 25 = +1. 50 = +3. Dále už bez dalších United Points.'},
  'earn-profile':{kicker:'DOPLNĚNÍ PROFILU',title:'+1 bod za kompletní profil',copy:'Profilová registrace musí být kompletní, historie zkontrolovaná, v Garage alespoň 1 auto a alespoň 5 schválených komunitních fotek. Newsletter není potřeba.'},
};
...
function closeAchievementDetail({restoreFocus=false}={}){
  if(!achievementPopover||achievementPopover.hidden)return;achievementPopover.hidden=true;
...
function openAchievementDetail(button){
  ...
  achievementTrigger=button;button.setAttribute('aria-expanded','true');
  $('[data-achievement-icon]',achievementPopover).innerHTML=achievementIcon(achievement.type);
  $('[data-achievement-tier]',achievementPopover).textContent=achievement.tier||'ACHIEVEMENT';
  ...
  const pointsEl=$('[data-achievement-points]',achievementPopover);pointsEl.hidden=!achievement.points;pointsEl.textContent=achievement.points?`+${achievement.points} ${pointWord(achievement.points)}`:'';
  const rect=button.getBoundingClientRect(),width=310,left=Math.max(12,Math.min(innerWidth-width-12,rect.left+rect.width/2-width/2)),top=Math.min(innerHeight-220,rect.bottom+10);
  achievementPopover.style.setProperty('--achievement-left',`${left}px`);achievementPopover.style.setProperty('--achievement-top',`${Math.max(12,top)}px`);achievementPopover.hidden=false;
}
...
function historySnsFields(item,{attendanceApproved=false}={}){
  ...
  const menuId=`history-category-${historyDomId(item.eventId)}`,categoryOptions=historyCategoryOptions().map(([value,label])=>`<button aria-selected="false" data-history-category-value="${esc(value)}" role="option" type="button">${esc(label)}</button>`).join('');
  return `<div class="history-sns-question"><span>${canAmend?'CHCEŠ DOPLNIT SHOW &amp; SHINE?':'SOUTĚŽIL/A JSI V SHOW &amp; SHINE?'}</span><input class="history-native-control" data-history-sns-input="" name="snsCompeted" type="checkbox" value="on"/><div aria-label="Show &amp; Shine" class="history-binary-choice" role="group"><button aria-pressed="true" class="is-selected" data-history-sns-choice="no" type="button">NE</button><button aria-pressed="false" data-history-sns-choice="yes" type="button">ANO</button></div></div><div class="history-sns-fields" data-history-sns-fields="" hidden><div class="history-category-field" data-history-category-field=""><span class="history-form-label">KATEGORIE</span><input data-history-category-input="" name="snsCategory" type="hidden" value=""/><button aria-controls="${menuId}" aria-expanded="false" aria-haspopup="listbox" class="history-category-trigger" data-history-category-trigger="" type="button"><span data-history-category-label="">Vyber kategorii</span><i aria-hidden="true">⌄</i></button><div aria-label="Kategorie Show &amp; Shine" class="history-category-menu" data-history-category-menu="" hidden id="${menuId}" role="listbox">${categoryOptions}</div></div><fieldset class="history-placement-field"><legend class="history-form-label">UMÍSTĚNÍ</legend><div class="history-placement-options"><label class="is-neutral"><input checked class="history-native-control" name="snsPlacement" type="radio" value=""/><span>BEZ TOP 3</span></label><label class="is-bronze"><input class="history-native-control" name="snsPlacement" type="radio" value="3"/><span>3. MÍSTO</span></label><label class="is-silver"><input class="history-native-control" name="snsPlacement" type="radio" value="2"/><span>2. MÍSTO</span></label><label class="is-gold"><input class="history-native-control" name="snsPlacement" type="radio" value="1"/><span>1. MÍSTO</span></label></div></fieldset><fieldset class="history-award-field"><legend class="history-form-label">DALŠÍ OCENĚNÍ</legend><div class="history-award-options"><label class="history-award-chip"><input class="history-native-control" name="snsBestOfBest" type="checkbox" value="on"/><span>◆ BEST OF THE BEST</span></label><label class="history-award-chip"><input class="history-native-control" name="snsBestExhaust" type="checkbox" value="on"/><span>◈ NEJ ZVUK VÝFUKU</span></label></div></fieldset></div>`;
}
...
function historyClaimForm(item,{focused=false}={}){
  const status=item.attendance?.status||'not_claimed',approved=status==='approved',pending=status==='pending';
  const cardClass=`history-editor-card ${focused?'is-focused-target':''}`,focusAttrs=`data-history-event="${esc(item.eventId)}" tabindex="-1"`;
  ...
  if(approved&&['pending','approved'].includes(item.showShine?.status))return `<article class="${cardClass}" ${focusAttrs}><header><strong>UNITED ${item.eventYear}</strong><span>DOCHÁZKA OVĚŘENA</span></header>${historyEvidenceMarkup(item)}${item.showShine?.status==='pending'?historySubmittedSummary(item,{attendance:false}):historySnsFields(item,{attendanceApproved:true})}</article>`;
  if(approved)return `<form class="${cardClass}" data-attendance-approved="true" data-history-claim-form="" ${focusAttrs}><input name="eventId" type="hidden" value="${esc(item.eventId)}"/><header><strong>UNITED ${item.eventYear}</strong><span>DOPLŇ SHOW &amp; SHINE</span></header><p>Schválenou účast nelze odebrat. Pokud jsi soutěžil/a, doplň výsledek ke kontrole.</p>${item.showShine?.reviewNote?`<div class="history-rejection-note"><b>DŮVOD S&S ZAMÍTNUTÍ</b><span>${esc(item.showShine.reviewNote)}</span></div>`:''}${historySnsFields(item,{attendanceApproved:true})}<button class="member-primary member-primary--compact" data-history-submit="" disabled type="submit">Odeslat S&amp;S ke kontrole</button></form>`;
  return `<form class="${cardClass}" data-attendance-approved="false" data-history-claim-form="" ${focusAttrs}><input name="eventId" type="hidden" value="${esc(item.eventId)}"/><header><strong>UNITED ${item.eventYear}</strong><span>${status==='rejected'?'DOPLŇ A ODEŠLI ZNOVU':'DOLOŽ SVOJI ÚČAST'}</span></header><p>Nahraj důkaz účasti a případně doplň Show &amp; Shine.</p>${status==='rejected'?`<div class="history-rejection-note"><b>DŮVOD ZAMÍTNUTÍ</b><span>${esc(item.attendance.reviewNote||'Doplň důkaz účasti.')}</span></div>`:''}<div class="history-claim-fields" data-history-claim-fields=""><label class="history-evidence-upload"><span>DŮKAZ ÚČASTI · POVINNÉ</span><input accept="image/jpeg,image/png,image/webp" data-history-evidence-input="" multiple name="evidence" type="file"/><b>Vybrat 1–4 soukromé fotografie</b><small>Fotky uvidíš pouze ty a oprávněný Admin.</small></label><div class="image-selection-preview-grid" data-history-evidence-preview=""></div>${historySnsFields(item)}<button class="member-primary member-primary--compact" data-history-submit="" disabled type="submit">Odeslat ke kontrole</button></div></form>`;
}
...
function renderHistoryEditor(focusedEventId=''){
  const list=$('[data-history-editor-list]');if(!list)return;historyPreviewControllers.forEach(controller=>controller.clear());historyPreviewControllers=[];
  const concluded=(data.club?.history||[]).filter(item=>item.concluded);if(focusedEventId)list.dataset.focusedEvent=focusedEventId;else delete list.dataset.focusedEvent;list.innerHTML=concluded.length?concluded.map(item=>historyClaimForm(item,{focused:String(item.eventId)===String(focusedEventId)})).join(''):'<article class="history-editor-empty"><b>Zatím tu není žádný ukončený ročník.</b><p>Ročník se nabídne až podle skutečného data konce uloženého na serveru.</p></article>';
  ...
  $$('[data-history-claim-form]',list).forEach(form=>{form.addEventListener('submit',submitHistoryEditorForm);syncHistorySubmitState(form)});void hydrateHistoryEvidence();
}
