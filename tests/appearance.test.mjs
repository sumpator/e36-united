...
test('all six HTML entries load blocking appearance bootstrap before their styles',()=>{for(const file of ['index','galerie','merch','o-nas','member','admin']){const html=readFileSync(new URL(`../${file}.html`,import.meta.url),'utf8');assert.match(html,/<script src="appearance.js\?v=20260923-theme1"><\/script>/);assert.ok(html.indexOf('appearance.js')<html.indexOf('rel="stylesheet"'));assert.match(html,/appearance.css\?v=20260923-theme1/);assert.match(html,/data-appearance-select/)}});
