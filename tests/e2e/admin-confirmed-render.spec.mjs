...
test('CONFIRMED reservation notes save independently, clear cleanly and keep history last',async({page},info)=>{
 ...
 try{
  ...
  await expect(history).toBeVisible();expect(await actions.evaluate((current,last)=>Boolean(current.compareDocumentPosition(last)&Node.DOCUMENT_POSITION_FOLLOWING),await history.elementHandle())).toBe(true);const pairedRows=await card.locator('.admin-reservation-drawer-grid').evaluate(grid=>{const sections=[...grid.children].filter(child=>child.matches('section'));return[[0,1],[2,3],[4,5],[6,7]].map(pair=>pair.map(index=>{const box=sections[index].getBoundingClientRect();return{y:box.y,height:box.height,visible:box.width>0&&box.height>0}}))});expect(pairedRows.map(row=>row.map(item=>item.visible))).toEqual([[true,true],[true,true],[true,true],[true,true]]);for(let index=0;index<pairedRows.length;index++){const[left,right]=pairedRows[index];expect(Math.abs(left.y-right.y)).toBeLessThanOrEqual(1);expect(Math.abs(left.height-right.height)).toBeLessThanOrEqual(1);if(index)expect(left.y).toBeGreaterThanOrEqual(pairedRows[index-1][0].y+pairedRows[index-1][0].height)}await drawer.locator('.admin-reservation-drawer-panel').evaluate(panel=>{panel.scrollTop=0});await page.screenshot({path:info.outputPath('admin-reservation-detail-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});expect(await card.locator('.admin-reservation-drawer-grid').evaluate(grid=>getComputedStyle(grid).gridTemplateColumns.split(' ').length)).toBe(1);expect(await drawer.locator('.admin-reservation-drawer-panel').evaluate(panel=>panel.scrollWidth<=panel.clientWidth)).toBe(true);await page.setViewportSize({width:1280,height:720});
