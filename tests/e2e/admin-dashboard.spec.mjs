...
for(const width of [1440,390])test('Stage 3 factory compositions, exact graphs, mobile navigation and screenshot '+width,async({page},info)=>{
 ...
 await card(page,'trend').locator('[data-dashboard-range]').selectOption('7');await card(page,'trend').locator('[data-dashboard-table] summary').click();await expect(card(page,'trend').locator('tbody tr')).toHaveCount(7);await expect(card(page,'trend').locator('tbody tr').first()).toContainText('2');
...
test('Stage 3 preferences save, reorder, size, independent compositions, cancel and confirmed reset',async({page},info)=>{
 ...
 await editor(page).locator('[data-widget-toggle="people"]').uncheck();await editor(page).locator('[data-edit-widget="trend"] [data-widget-size]').selectOption('compact');await editor(page).locator('[data-edit-widget="trend"] [data-widget-move="-1"]').click();
 ...
 await page.reload();await expect(card(page,'people')).toHaveCount(0);await expect(page.locator('[data-dashboard-edit]')).toBeEnabled();await edit(page);await editor(page).locator('[data-widget-toggle="people"]').check();page.once('dialog',d=>d.accept());await page.locator('[data-dashboard-cancel]').click();expect(c.stored()).toEqual(first);
 await page.locator('[data-dashboard-composition]').selectOption('onsite');await edit(page);await editor(page).locator('[data-widget-toggle="reservations"]').uncheck();await save(page);const beforeReset=c.stored();
...
test('Stage 3 dirty preferences reject newer device revision, retain draft and never overwrite by refresh',async({page})=>{
 const c=await fixture(page);await open(page);await edit(page);await editor(page).locator('[data-widget-toggle="people"]').uncheck();const draft=await editor(page).locator('[name=configuration]').inputValue();
...
for(const mode of ['lost','undelivered'])test('Stage 3 preference outcome recovery '+mode,async({page})=>{
 const c=await fixture(page);await open(page);await edit(page);await editor(page).locator('[data-widget-toggle="people"]').uncheck();c.mode=mode;await page.locator('[data-dashboard-save]').click();await expect(editor(page)).toHaveAttribute('data-operation-state','outcome_unknown');expect(c.writes).toHaveLength(1);c.mode='';
...
test('Stage 3 unavailable preferences cannot overwrite saved work with factory defaults',async({page})=>{
 ...
 c.mode='';await page.locator('[data-refresh]').click();await expect(page.locator('[data-dashboard-edit]')).toBeEnabled();await edit(page);expect(JSON.parse(await editor(page).locator('[name=configuration]').inputValue()).compositions.preparation.widgets).toContainEqual({id:'future-widget',size:'wide'});await expect(editor(page)).not.toContainText('Neznámý widget');expect(c.stored().value).toEqual(saved);clean(c);c.r.db.close();
});
