...
test('approved reservation change supports sold-out inquiry, compact layouts and persistent acknowledgement',async({page},info)=>{
  ...
  await expect(detailCard.locator('.reservation-unified-head')).toHaveCount(0);
  await expect(detailCard.locator('.reservation-hero-strip')).toBeVisible();
  await expect(detailCard.locator('[data-reservation-summary]')).toBeVisible();
  await expect(detailCard.locator('[data-member-weekend-planner]')).toBeHidden();
  await expect(page.locator('[data-reservation-change-modal]')).toBeHidden();
  ...
  const detailGeometry=await detailCard.evaluate(node=>{
    ...
    return{columns:getComputedStyle(grid).gridTemplateColumns.split(' ').length,gap:rightRect.left-leftRect.right,left:leftRect,right:rightRect,leftRatio:leftRect.width/gridRect.width,tileRects:tiles.map(tile=>tile.getBoundingClientRect()),valueStyles:tiles.map(tile=>{const style=getComputedStyle(tile.querySelector('b'));return{fontSize:parseFloat(style.fontSize),fontWeight:Number(style.fontWeight),lineHeight:parseFloat(style.lineHeight)}}),leftRadius:parseFloat(getComputedStyle(left).borderRadius),rightRadius:parseFloat(getComputedStyle(right).borderRadius)};
  });
  ...
  for(const style of detailGeometry.valueStyles){expect(style.fontSize).toBe(16);expect(style.fontWeight).toBeGreaterThanOrEqual(700);expect(style.lineHeight/style.fontSize).toBeGreaterThanOrEqual(1.2);expect(style.lineHeight/style.fontSize).toBeLessThanOrEqual(1.3)}
  ...
  const stayValue=page.locator('[data-reservation-summary]>div').nth(1).locator('.registration-summary-value'),accommodationValue=page.locator('[data-reservation-summary]>.member-summary-accommodation .registration-summary-value');
  await expect(stayValue.locator('b')).toHaveText('Sobota → neděle');await expect(stayValue.locator('span')).toHaveText('1 noc');await expect(accommodationValue.locator('b')).toHaveText('2 osoby');await expect(accommodationValue.locator('span')).toHaveText('1× Chatka Premium');
  if(process.env.E36_REVIEW_SCREENSHOTS){const reviewImage=detailCard.locator('.member-saved-accommodation-visual img');await reviewImage.evaluate(image=>{image.src='assets/images/program/saturday.webp'});await expect.poll(()=>reviewImage.evaluate(image=>image.complete&&image.naturalWidth>0)).toBe(true)}
  await page.screenshot({path:info.outputPath('reservation-detail-desktop.png'),fullPage:true});
  if(process.env.E36_REVIEW_SCREENSHOTS){await page.screenshot({path:'test-results/member-ux-final/approved-detail-1440.png',fullPage:true});await page.setViewportSize({width:390,height:844});await page.screenshot({path:'test-results/member-ux-final/approved-detail-390.png',fullPage:true});await detailCard.locator('[data-reservation-card]').screenshot({path:'test-results/member-ux-final/approved-summary-390.png'});await page.setViewportSize({width:1440,height:1000})}
  ...
  const form=page.locator('[data-reservation-form]'),feedback=page.locator('[data-reservation-form-status]'),changeModal=page.locator('[data-reservation-change-modal]');await clickReady(page.locator('[data-request-change]'));await expect(changeModal).toBeVisible();await expect(form).toHaveClass(/is-editing/);await expect(form.locator('.reservation-form-row')).toHaveCount(3);await expect(page.locator('[data-reservation-summary]')).toContainText('Chatka Premium');await expect(form.locator('[name=accommodationOptionId]')).toBeEnabled();await expect(form.locator('[name=accommodationOptionId] option[value="cabin-standard"]')).toContainText('VYPRODÁNO — LZE POŽÁDAT INDIVIDUÁLNĚ');await form.locator('[name=accommodationOptionId]').selectOption('cabin-standard');await expect(page.locator('[data-accommodation-availability]')).toContainText('individuální žádost');await form.locator('[data-member-crew-plus]').click();await expect(form.locator('[name=crew]')).toHaveValue('3');
  await expect(changeModal.locator('.planner-step--car')).toBeHidden();
  if(process.env.E36_REVIEW_SCREENSHOTS){await changeModal.locator('.reservation-change-planner-dialog').evaluate(node=>{node.scrollTop=0});await page.screenshot({path:'test-results/member-ux-final/approved-change-modal-1440.png',fullPage:false})}
  await page.setViewportSize({width:390,height:844});
  const mobileOrder=await page.evaluate(()=>{const status=document.querySelector('.reservation-status-card').getBoundingClientRect(),left=document.querySelector('[data-reservation-card]').getBoundingClientRect(),right=document.querySelector('.reservation-control-stack').getBoundingClientRect(),payment=document.querySelector('[data-reservation-payment-detail]').getBoundingClientRect(),tileNodes=[...document.querySelectorAll('[data-reservation-summary]>div')],tiles=tileNodes.map(node=>node.getBoundingClientRect()),valueSizes=tileNodes.map(node=>parseFloat(getComputedStyle(node.querySelector('b')).fontSize));return{statusBottom:status.bottom,leftTop:left.top,leftBottom:left.bottom,leftLeft:left.left,leftRight:left.right,rightTop:right.top,rightBottom:right.bottom,paymentTop:payment.top,tiles,valueSizes,overflow:document.documentElement.scrollWidth>innerWidth}});
  ...
  for(const fontSize of mobileOrder.valueSizes)expect(fontSize).toBeGreaterThanOrEqual(15);
  const mobileSummaryValues=detailCard.locator('[data-reservation-summary] .registration-summary-value');await expect(mobileSummaryValues).toHaveCount(4);for(const value of await mobileSummaryValues.all())await expect(value).toBeVisible();
  ...
  await page.screenshot({path:info.outputPath('reservation-detail-mobile-390.png'),fullPage:false});
  if(process.env.E36_REVIEW_SCREENSHOTS){const dialog=changeModal.locator('.reservation-change-planner-dialog'),crewStep=changeModal.locator('.planner-step--crew'),note=changeModal.locator('.member-planner-note');await dialog.evaluate(node=>{node.scrollTop=0});await page.screenshot({path:'test-results/member-ux-final/approved-change-modal-390.png',fullPage:false});await crewStep.scrollIntoViewIfNeeded();await page.screenshot({path:'test-results/member-ux-final/approved-planner-crew-390.png',fullPage:false});await note.scrollIntoViewIfNeeded();await page.screenshot({path:'test-results/member-ux-final/approved-planner-bottom-390.png',fullPage:false});await dialog.evaluate(node=>{node.scrollTop=node.scrollHeight})}
  await page.setViewportSize({width:360,height:520});await form.locator('[name=note]').focus();await form.locator('[name=note]').scrollIntoViewIfNeeded();const compactGeometry=await changeModal.evaluate(node=>{const dialog=node.querySelector('.reservation-change-planner-dialog'),note=node.querySelector('.member-planner-note').getBoundingClientRect(),actions=node.querySelector('.reservation-edit-actions').getBoundingClientRect();return{overflow:dialog.scrollWidth>dialog.clientWidth+1,noteBottom:note.bottom,actionsTop:actions.top,actionsBottom:actions.bottom,viewportHeight:innerHeight}});expect(compactGeometry.overflow).toBe(false);expect(compactGeometry.noteBottom).toBeLessThanOrEqual(compactGeometry.actionsTop+1);expect(compactGeometry.actionsBottom).toBeLessThanOrEqual(compactGeometry.viewportHeight+1);
