...
test('approved reservation change supports sold-out inquiry, compact layouts and persistent acknowledgement',async({page},info)=>{
  ...
  await page.screenshot({path:info.outputPath('reservation-change-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.screenshot({path:info.outputPath('reservation-change-mobile-390.png'),fullPage:true});await page.setViewportSize({width:1440,height:1000});await clickReady(page.locator('[data-reservation-submit]'));
