...
test('approved reservation change supports sold-out inquiry, compact layouts and persistent acknowledgement',async({page},info)=>{
  ...
  await page.goto('/member.html?section=reservation');await expect(page.locator('[data-reservation-payment-detail]')).toContainText('Zaplaceno');const statusCard=page.locator('.reservation-status-card'),detailCard=page.locator('.reservation-unified-card');await expect(statusCard).toContainText('STAV REZERVACE');await expect(detailCard).not.toContainText('STAV REZERVACE');const statusBox=await statusCard.boundingBox(),detailBox=await detailCard.boundingBox();expect(detailBox.y-statusBox.y-statusBox.height).toBeGreaterThanOrEqual(10);const heroBox=await page.locator('.reservation-hero-strip').boundingBox();expect(heroBox.height).toBeGreaterThanOrEqual(160);expect(heroBox.height).toBeLessThanOrEqual(220);const detailGeometry=await detailCard.evaluate(node=>{const grid=node.querySelector('.reservation-detail-grid'),left=node.querySelector('[data-reservation-card]'),right=node.querySelector('.reservation-control-stack'),tiles=[...node.querySelectorAll('[data-reservation-summary]>div')];return{columns:getComputedStyle(grid).gridTemplateColumns.split(' ').length,left:left.getBoundingClientRect(),right:right.getBoundingClientRect(),tileHeights:tiles.map(tile=>tile.getBoundingClientRect().height)}});expect(detailGeometry.columns).toBe(2);expect(Math.abs(detailGeometry.left.y-detailGeometry.right.y)).toBeLessThanOrEqual(1);expect(Math.max(...detailGeometry.tileHeights)).toBeLessThanOrEqual(76);await expect(page.locator('[data-reservation-summary]>div')).toHaveCount(5);await expect(detailCard.locator('[data-reservation-car-choice]')).toBeVisible();await expect(detailCard.locator('[data-reservation-car-choice]')).toContainText('Změnit auto');await expect(detailCard.locator('[data-reservation-car-choice]')).not.toContainText('bez nového schvalování');await expect(page.locator('[data-reservation-form-car]')).toBeHidden();await expect(detailCard.locator('.member-saved-accommodation-visual')).toHaveCount(1);await expect(page.locator('[data-reservation-summary] .accommodation-visual')).toHaveCount(0);await page.screenshot({path:info.outputPath('reservation-detail-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});const mobileOrder=await page.evaluate(()=>{const status=document.querySelector('.reservation-status-card').getBoundingClientRect(),summary=document.querySelector('[data-reservation-summary]').getBoundingClientRect(),controls=document.querySelector('.reservation-control-stack').getBoundingClientRect();return{statusBottom:status.bottom,summaryTop:summary.top,summaryBottom:summary.bottom,controlsTop:controls.top,overflow:document.documentElement.scrollWidth>innerWidth}});expect(mobileOrder.overflow).toBe(false);expect(mobileOrder.statusBottom).toBeLessThan(mobileOrder.summaryTop);expect(mobileOrder.summaryBottom).toBeLessThanOrEqual(mobileOrder.controlsTop+1);await page.screenshot({path:info.outputPath('reservation-detail-mobile-390.png'),fullPage:true});await page.setViewportSize({width:1440,height:1000});
  ...
  await page.screenshot({path:info.outputPath('reservation-change-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);expect(await detailCard.locator('.reservation-detail-grid').evaluate(node=>getComputedStyle(node).gridTemplateColumns.split(' ').length)).toBe(1);await page.setViewportSize({width:1440,height:1000});await clickReady(page.locator('[data-reservation-submit]'));
