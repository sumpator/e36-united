...
async function waitForDisplayedProfileBlobImages(profileModal){
  await expect(profileModal.locator('.club-profile-hero [data-club-profile-private-photo]')).toHaveAttribute('src',/^blob:/);
  await expect.poll(()=>profileModal.locator('[data-club-profile-private-photo][src^="blob:"]:visible').evaluateAll(images=>({
    count:images.length,
    ready:images.every(image=>image.complete&&image.naturalWidth>0),
  }))).toEqual({count:2,ready:true});
}

const scrollOwnershipState=page=>page.evaluate(()=>({
  restoration:history.scrollRestoration,
  documentBehavior:document.documentElement.style.getPropertyValue('scroll-behavior'),
  documentPriority:document.documentElement.style.getPropertyPriority('scroll-behavior'),
...
test('MEMBER CLUB onboarding acknowledgement is isolated per member and completion uses required steps only',async({browser})=>{
  ...
  const complete=await context.newPage();await complete.setViewportSize({width:1440,height:980});await prepareE2ePage(complete,{authenticated:true,registrationOpen:false,clubProfiles,approvedGallery,clubPayload:{clubMembers,profileCompletion:{requiredFields:true,historyReviewed:false,hasCar:true,approvedPhotos:0,complete:false},approvedPhotoCount:0},member:{id:'member-complete',memberCode:'EU-C',name:'Complete United',email:'complete@example.test'}});await complete.goto('/member.html');await expect(complete.locator('[data-onboarding-intro-modal]')).toBeHidden();await expect(complete.locator('[data-united-onboarding]')).toBeHidden();const confirmation=complete.locator('[data-profile-complete]');await expect(confirmation).toBeVisible();await expect(confirmation).toHaveText('Profil kompletní ✓');await complete.locator('[aria-label="United Member Card"]').screenshot({path:'test-results/member-profile-complete.png'});await complete.close();await context.close();
});
