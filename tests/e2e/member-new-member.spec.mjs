...
async function logoutMobile(page,{discardPlanner=false}={}) {
  if(discardPlanner)page.once('dialog',dialog=>dialog.accept());
...
test('NEW MEMBER reservation entry is explicit, responsive and never duplicates an existing reservation',async({page,context},info)=>{
  ...
  await page.setViewportSize({width:390,height:844});await expect(page.locator('button[data-main-member-section="reservation"]')).toHaveText('Rezervace');expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.locator('.member-sidebar [data-member-section="overview"]').click();await page.screenshot({path:info.outputPath('new-member-overview-mobile-390.png'),fullPage:true});await card.click();await expect(entry).toBeFocused();await expectReservationGeometry(page);await page.screenshot({path:info.outputPath('new-member-reservation-mobile-390.png'),fullPage:true});
  await page.locator('.member-sidebar [data-member-section="garage"]').click();await expect(page.locator('[data-member-planner-modal]')).toBeHidden();await page.locator('[data-open-car]').click();const carForm=page.locator('[data-car-form]');await expect(page.locator('.image-upload-recommendation')).toHaveText('Fotografie není povinná, ale doporučujeme ji nahrát.');await carForm.locator('[name="nickname"]').fill('Bez fotky');await carForm.locator('[name="model"]').fill('316i');await carForm.locator('[type="submit"]').click();await expect.poll(()=>observations.carWrites.length).toBe(1);await expect(page.locator('[data-car-modal]')).toBeHidden();expect(observations.requests.some(value=>/\/photos$/.test(value))).toBe(false);
...
test('NEW MEMBER account switch never carries a private reservation note to another UID',async({page})=>{
  ...
  await page.goto('/member.html');await closeOnboarding(page);await page.locator('[data-reservation-overview-card]').click();const modal=page.locator('[data-member-planner-modal]'),note=page.locator('[data-reservation-form] [name="note"]'),logout=page.locator('.member-main-mobile-logout');await note.fill('PŘIJEDE NÁS 6');await page.evaluate(()=>localStorage.setItem('e36UnitedReservationDraftV20',JSON.stringify({note:'PŘIJEDE NÁS 6'})));page.once('dialog',dialog=>dialog.dismiss());await page.locator('.menu-btn').click();await expect(modal).toBeVisible();await expect(note).toHaveValue('PŘIJEDE NÁS 6');await expect(logout).not.toBeInViewport();await logoutMobile(page,{discardPlanner:true});
  ...
  await logoutMobile(page,{discardPlanner:true});await observations.switchMember({id:'member-a',email:'a@example.test',name:'Member A',nickname:'A'});await login(page,'a@example.test');await closeOnboarding(page);await page.locator('.menu-btn').click();await page.locator('[data-main-member-section="reservation"]').click();await page.locator('[data-member-plan-open]').click();await expect(modal).toBeVisible();await expect(note).toHaveValue('');expect(observations.reservationWrites).toEqual([]);expectNoUnexpectedClientErrors(observations);
});
