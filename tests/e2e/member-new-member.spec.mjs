...
async function closePlanner(page,{discardChanges=false}={}) {
  ...
  await close.click();
  if(discardChanges)expect(discardMessage).toBe('Zahodit neuložené změny předběžné registrace?');
...
test('NEW MEMBER reservation entry is explicit, responsive and never duplicates an existing reservation',async({page,context},info)=>{
  ...
  await expect(card).toBeVisible();await expect(card.locator('[data-reservation-overview-label]')).toHaveText('REGISTRUJ SE NA UNITED');await expect(card.locator('[data-reservation-overview-copy]')).toBeEmpty();await expect(card.locator('[data-reservation-overview-action]')).toContainText('Začít');
  await expect(page.locator('.member-sidebar [data-member-section="reservation"]')).toContainText('Registrace');
  ...
  await card.click();const entry=page.locator('[data-member-planner-title]');await expect(page.locator('[data-member-panel="reservation"]')).toBeVisible();await expect(entry).toBeFocused();await expect(entry).toBeInViewport();await expect(page.locator('[data-reservation-state-label]')).toHaveText('REGISTRUJ SE NA UNITED');await expect(page.locator('[data-reservation-description]')).toBeEmpty();await expectReservationGeometry(page);
  await page.screenshot({path:info.outputPath('new-member-reservation-desktop.png'),fullPage:true});
  await page.setViewportSize({width:390,height:844});await expect(page.locator('button[data-main-member-section="reservation"]')).toHaveText('Registrace');expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await closePlanner(page);await page.locator('.member-sidebar [data-member-section="overview"]').click();await page.screenshot({path:info.outputPath('new-member-overview-mobile-390.png'),fullPage:true});await card.click();await expect(entry).toBeFocused();await expectReservationGeometry(page);await page.screenshot({path:info.outputPath('new-member-reservation-mobile-390.png'),fullPage:true});
  ...
  const closed=await context.newPage();const closedObservations=await prepareE2ePage(closed,{authenticated:true,registrationOpen:false,preliminaryEnabled:true,reservation:null,cars:[]});await closed.goto('/member.html');await closeOnboarding(closed);const closedCard=closed.locator('[data-reservation-overview-card]');await expect(closedCard.locator('[data-reservation-overview-label]')).toHaveText('REGISTRUJ SE NA UNITED');await expect(closedCard.locator('[data-reservation-overview-copy]')).toHaveText('Zatím přijímáme předběžné registrace.');await expect(closedCard.locator('[data-reservation-overview-action]')).toContainText('Začít');await closedCard.click();await expect(closed.locator('[data-reservation-form] [name="note"]')).toBeEnabled();await closed.locator('[data-member-stay="2"]').click();await expect(closed.locator('[data-reservation-form] [name="arrival"]')).toHaveValue('Jen na otočku');await closed.locator('[data-reservation-form] [name="note"]').fill('Připraveno lokálně');await closed.locator('[data-reservation-submit]').click();await expect(closed.locator('[data-reservation-form-status]')).toContainText('Předběžná registrace byla uložena.');expect(closedObservations.reservationWrites).toEqual([]);expect(closedObservations.preliminaryWrites).toHaveLength(1);expectNoUnexpectedClientErrors(closedObservations);await closed.close();

  const existing=await context.newPage();const existingObservations=await prepareE2ePage(existing,{authenticated:true,registrationOpen:true,reservation:{id:'existing',eventId:'united-2026',eventYear:2026,title:'E36 United 2026',arrival:'Pátek',crew:2,accommodation:'Bez ubytování',accommodationUnits:0,showShine:'Ne',note:'',status:'pending'},cars:[]});await existing.goto('/member.html');await closeOnboarding(existing);const existingCard=existing.locator('[data-reservation-overview-card]');await expect(existingCard).toContainText('ČEKÁ NA SCHVÁLENÍ');await expect(existingCard).toContainText('Otevřít registraci');await expect(existingCard).not.toContainText('Začít');await expect(existingCard).not.toHaveAttribute('data-reservation-form-jump','');expectNoUnexpectedClientErrors(existingObservations);await existing.close();
