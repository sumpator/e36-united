...
async function closePlanner(page,{discardChanges=false}={}) {
  ...
  await close.click();
  if(discardChanges)expect(discardMessage).toBe('Zahodit neuložené změny předběžné rezervace?');
...
test('NEW MEMBER reservation entry is explicit, responsive and never duplicates an existing reservation',async({page,context},info)=>{
  ...
  await expect(card).toBeVisible();await expect(card.locator('[data-reservation-overview-label]')).toHaveText('REZERVACE JSOU OTEVŘENÉ');await expect(card.locator('[data-reservation-overview-copy]')).toHaveText('Vyber příjezd, posádku, Show & Shine a případné ubytování.');await expect(card.locator('[data-reservation-overview-action]')).toContainText('Vytvořit rezervaci');
  ...
  await card.click();const entry=page.locator('[data-member-planner-title]');await expect(page.locator('[data-member-panel="reservation"]')).toBeVisible();await expect(entry).toBeFocused();await expect(entry).toBeInViewport();await expect(page.locator('[data-reservation-state-label]')).toHaveText('REZERVACE JSOU OTEVŘENÉ');await expect(page.locator('[data-reservation-description]')).toHaveText('Rezervace na E36 United 2026 jsou otevřené. Vyplň Weekend Planner a odešli rezervaci ke schválení.');await expectReservationGeometry(page);
  ...
  const closed=await context.newPage();const closedObservations=await prepareE2ePage(closed,{authenticated:true,registrationOpen:false,preliminaryEnabled:true,reservation:null,cars:[]});await closed.goto('/member.html');await closeOnboarding(closed);const closedCard=closed.locator('[data-reservation-overview-card]');await expect(closedCard.locator('[data-reservation-overview-label]')).toHaveText('PŘIPRAV PŘEDBĚŽNOU REZERVACI');await expect(closedCard.locator('[data-reservation-overview-copy]')).toHaveText('Ulož předběžnou rezervaci bez blokace kapacity a bez platební povinnosti.');await expect(closedCard.locator('[data-reservation-overview-action]')).toContainText('Vytvořit předběžnou rezervaci');await closedCard.click();await expect(closed.locator('[data-reservation-form] [name="note"]')).toBeEnabled();await closed.locator('[data-reservation-form] [name="arrival"]').selectOption('Jen na otočku');await closed.locator('[data-reservation-form] [name="note"]').fill('Připraveno lokálně');await closed.locator('[data-reservation-submit]').click();await expect(closed.locator('[data-reservation-form-status]')).toContainText('Předběžná rezervace byla uložena.');expect(closedObservations.reservationWrites).toEqual([]);expect(closedObservations.preliminaryWrites).toHaveLength(1);expectNoUnexpectedClientErrors(closedObservations);await closed.close();
