...
test('Merch catalog administration, account defaults and payment tabs share authoritative orders',async({page,context})=>{
 ...
 await f.locator('details:has(>[name=id]),details:has(>label>[name=id])').locator(':scope>summary').click();await f.locator('[data-variant] details:has(>label>[name=variantId])').locator('summary').click();
 for(const [name,value]of Object.entries({id:'ui-product',name:'UI produkt',category:'Nová UI kategorie',variantId:'ui-blue',color:'Modrá',price:'690'}))await f.locator(`[name=${name}]`).fill(value);
 for(const size of ['S','M','L','XL'])await f.locator(`[name=size][value=${size}]`).check();await f.getByText('Vybrat z uložených fotografií',{exact:true}).click();await f.locator('input[type=radio][value=polo-navy]').check();await f.locator('[name=availability]').selectOption('available');await f.locator('[name=status]').selectOption('published');await f.getByRole('button',{name:'Uložit produkt',exact:true}).click();await expect(page.locator('[data-edit-product="ui-product"]')).toBeVisible();
 ...
 await member.goto('/member.html?section=account');await member.locator('[data-address-edit]').click();const address=member.locator('[data-merch-address] form');await expect(address).toBeVisible();
 ...
 await address.getByRole('button',{name:'Uložit dodací údaje'}).click();await expect(member.locator('[data-merch-address] [role=status]')).toContainText('uloženy');
 ...
 await page.goto('/admin.html?section=payments');await page.locator('[data-finance-tab="merch"]').click();await page.locator(`[data-payment-order="${o.id}"]`).click();await expect(page.locator('[data-payment-detail]')).toContainText(o.number);await page.locator('[data-finance-tab="sraz"]').click();await expect(page.locator('[data-sraz-payments]')).toBeVisible();
