...
for(const width of [1440,390])test(`PLAN shared modal saves without a car and survives a new session ${width}`,async({page},info)=>{
 ...
 await modal.locator('[data-member-show="Možná"]').click();await form.locator('[name="note"]').fill('Zájem bez závazku');await form.locator('[name="preliminaryCrewDetails"]').fill('Řidič\nDoprovod');
