...
test('PLAN handoff stays a draft until an explicit save creates the active plan',async({page})=>{
 ...
 const banner=page.locator('[data-planner-handoff]');await expect(banner).toBeVisible();await expect(banner.locator('[data-planner-handoff-title]')).toHaveText('DOKONČI SVŮJ PLÁN');await expect(banner.locator('[data-planner-handoff-copy]')).toHaveText('Výběr z Weekend Planneru jsme přenesli. Zkontroluj ho a ulož jako nezávazný plán.');const save=page.locator('[data-planner-handoff-continue]');await expect(save).toBeEnabled();await expect(save).toContainText('ZKONTROLOVAT A ULOŽIT PLÁN');await expect(banner).not.toContainText('TVŮJ PLÁN JE PŘIPRAVENÝ');await expect(banner).not.toContainText('Výběr z Weekend Planneru jsme uložili');expect(observations.preliminaryWrites).toHaveLength(0);
 await save.click();const modal=page.locator('[data-member-planner-modal]');await expect(modal).toBeVisible();await modal.locator('[data-reservation-submit]').click();await expect(modal).toBeHidden();await expect(page.locator('[data-preliminary-copy]')).toHaveText('Tvůj plán máme. Jakmile spustíme rezervace, dáme ti vědět.');expect(observations.preliminaryWrites).toHaveLength(1);expect(observations.reservationWrites).toHaveLength(0);expectNoUnexpectedClientErrors(observations);await page.close();
});
