...
for(const width of [1440,390])test(`PLAN shared modal saves without a car and survives a new session ${width}`,async({page},info)=>{
 ...
 await expect(page.locator('[data-reservation-status-card], .reservation-status-card')).toContainText('Předběžnou rezervaci si můžeš uložit bez blokace kapacity');await expect(page.locator('[data-preliminary-detail]')).toBeVisible();await page.locator('[data-member-plan-open]').click();await expect(modal).toBeVisible();
 await expect(submit).toContainText('Uložit předběžnou rezervaci');await expect(form.locator('[name="carId"]')).toHaveValue('');await form.locator('[name="arrival"]').selectOption('Jen na otočku');
 ...
 await review(`member-plan-edit-${width}`);await submit.click();await expect(modal).toBeHidden();await expect(page.locator('[data-preliminary-copy]')).toHaveText('Nezávazná, bez zajištěné kapacity. Po otevření rezervací ji zkontroluješ a odešleš ke schválení.');await expect(page.locator('[data-member-plan-disclaimer]')).toHaveText('Předběžná rezervace · bez rezervované kapacity');await expect(page.locator('[data-reservation-summary]>div')).toHaveCount(5);
 ...
 await page.reload();await expect(page.locator('[data-preliminary-copy]')).toHaveText('Nezávazná, bez zajištěné kapacity. Po otevření rezervací ji zkontroluješ a odešleš ke schválení.');const editPlan=page.locator('[data-member-plan-open]');await stableScroll(page);
 ...
 await expect(submit).toContainText('Uložit změny předběžné rezervace');
 ...
 await form.locator('[name="note"]').fill('Upravený zájem');await submit.click();await expect(page.locator('[data-preliminary-copy]')).toHaveText('Nezávazná, bez zajištěné kapacity. Po otevření rezervací ji zkontroluješ a odešleš ke schválení.');
...
test('PLAN handoff stays a draft until an explicit save creates the active plan',async({page})=>{
 ...
 const banner=page.locator('[data-planner-handoff]');await expect(banner).toBeVisible();await expect(banner.locator('[data-planner-handoff-title]')).toHaveText('DOKONČI PŘEDBĚŽNOU REZERVACI');await expect(banner.locator('[data-planner-handoff-copy]')).toHaveText('Výběr z Weekend Planneru jsme přenesli. Zkontroluj ho a ulož jako předběžnou rezervaci.');const save=page.locator('[data-planner-handoff-continue]');await expect(save).toBeEnabled();await expect(save).toContainText('ZKONTROLOVAT A ULOŽIT PŘEDBĚŽNOU REZERVACI');expect(observations.preliminaryWrites).toHaveLength(0);
 await save.click();const modal=page.locator('[data-member-planner-modal]');await expect(modal).toBeVisible();await modal.locator('[data-reservation-submit]').click();await expect(modal).toBeHidden();await expect(page.locator('[data-preliminary-copy]')).toHaveText('Nezávazná, bez zajištěné kapacity. Po otevření rezervací ji zkontroluješ a odešleš ke schválení.');expect(observations.preliminaryWrites).toHaveLength(1);expect(observations.reservationWrites).toHaveLength(0);expectNoUnexpectedClientErrors(observations);await page.close();
});
...
test('PLAN disabled configuration never presents a draft as saved',async({page})=>{
 ...
 const state=page.locator('[data-reservation-workbench]');await expect(state).toContainText('Rezervace na E36 United 2026 nyní nejsou otevřené.');await expect(state).not.toContainText('PŘEDBĚŽNÁ REZERVACE · ULOŽENÁ');await expect(page.locator('[data-member-plan-open]')).toBeHidden();await expect(page.locator('[data-reservation-form]')).toBeHidden();expectNoUnexpectedClientErrors(observations);await page.close();
});
