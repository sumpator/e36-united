...
SELECT
 COALESCE((SELECT SUM(status IN ('pending','approved')) FROM r),0) AS reservations,
 COALESCE((SELECT SUM(CASE WHEN status IN ('pending','approved') THEN crew ELSE 0 END) FROM r),0) AS people,
 COALESCE((SELECT COUNT(DISTINCT CASE WHEN status IN ('pending','approved') THEN car_id END) FROM r),0) AS cars,
 COALESCE((SELECT SUM(status='pending') FROM r),0) AS status_pending,
 COALESCE((SELECT SUM(status='approved') FROM r),0) AS status_approved,
 COALESCE((SELECT SUM(status='rejected') FROM r),0) AS status_rejected,
 COALESCE((SELECT SUM(status='cancelled') FROM r),0) AS status_cancelled,
 COALESCE((SELECT SUM(status='draft') FROM r),0) AS status_draft,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND attendance_type='full_weekend') FROM r),0) AS attendance_full_weekend,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND attendance_type='saturday_only') FROM r),0) AS attendance_saturday_only,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND attendance_type='day_visit') FROM r),0) AS attendance_day_visit,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND show_shine='Ano') FROM r),0) AS show_yes,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND show_shine='Ne') FROM r),0) AS show_no,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND show_shine='Možná') FROM r),0) AS show_maybe,
 COALESCE((SELECT SUM(CASE WHEN status IN ('pending','approved') THEN amount_due_czk ELSE 0 END) FROM r),0) AS amount_due,
 COALESCE((SELECT SUM(amount_paid_czk) FROM r),0) AS amount_paid,
 COALESCE((SELECT SUM(CASE WHEN status IN ('pending','approved') THEN amount_paid_czk ELSE 0 END) FROM r),0) AS active_paid,
 COALESCE((SELECT SUM(CASE WHEN status NOT IN ('pending','approved') THEN amount_paid_czk ELSE 0 END) FROM r),0) AS inactive_paid,
 COALESCE((SELECT SUM(CASE WHEN status IN ('pending','approved') THEN MAX(amount_due_czk-amount_paid_czk,0) ELSE 0 END) FROM r),0) AS remaining,
 COALESCE((SELECT SUM(CASE WHEN status IN ('pending','approved') THEN MIN(amount_due_czk,amount_paid_czk) ELSE 0 END) FROM r),0) AS applied,
 COALESCE((SELECT SUM(MAX(amount_paid_czk-amount_due_czk,0)) FROM r),0) AS overpayments,
 COALESCE((SELECT SUM(amount_due_czk>0 AND amount_paid_czk=0) FROM r),0) AS pay_unpaid,
 COALESCE((SELECT SUM(amount_paid_czk>0 AND amount_paid_czk<amount_due_czk) FROM r),0) AS pay_underpaid,
 COALESCE((SELECT SUM(amount_due_czk>0 AND amount_paid_czk=amount_due_czk) FROM r),0) AS pay_paid,
 COALESCE((SELECT SUM(amount_paid_czk>amount_due_czk) FROM r),0) AS pay_overpaid,
 COALESCE((SELECT SUM(amount_paid_czk=0 AND amount_due_czk=0) FROM r),0) AS pay_none,
 COALESCE((SELECT SUM(overdue) FROM r),0) AS pay_overdue,
 COALESCE((SELECT SUM(overdue OR amount_paid_czk>amount_due_czk) FROM r),0) AS payment_attention,
 COALESCE((SELECT SUM(status='pending' OR overdue OR amount_paid_czk>amount_due_czk) FROM r),0) AS reservation_attention,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND accommodation_units>0 AND NOT EXISTS(SELECT 1 FROM reservation_accommodation ra WHERE ra.reservation_id=r.id)) FROM r),0) AS legacy_allocations,
 COALESCE((SELECT SUM(status IN ('pending','approved') AND accommodation='Bez ubytování') FROM r),0) AS no_accommodation,
